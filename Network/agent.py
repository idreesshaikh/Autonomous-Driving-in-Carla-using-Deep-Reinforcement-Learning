...
class Agent(object):
    ...
    def __init__(self, n_actions, double=True):
        ...
        self.replay_buffer = ReplayBuffer(
            MEMORY_SIZE, (128, 128, 3), (98), (2), n_actions)
        self.q_network_eval = DDQnetwork(n_actions, MODEL_ONLINE)
        self.q_network_target = DDQnetwork(n_actions, MODEL_TARGET)

    def save_transition(self, visual_state, raw_state, nav_data, action, reward, new_visual_state, new_raw_data, new_nav_data, done):
        self.replay_buffer.save_transition(
            visual_state, raw_state, nav_data, action, reward, new_visual_state, new_raw_data, new_nav_data, done)

    def pick_action(self, visual_state, raw_state, nav_data):
        if np.random.random() > self.epsilon:
            ...
            raw_state = torch.tensor([raw_state], dtype=torch.float32).to(
                self.q_network_eval.device)
            nav_data = torch.tensor([nav_data], dtype=torch.float32).to(
                self.q_network_eval.device)
            actions = self.q_network_eval.forward(
                visual_state, raw_state, nav_data)
    ...
    def train(self):
        ...
        visual_state, raw_state, nav_data, action, reward, new_visual_state, new_raw_data, new_nav_data, done = self.replay_buffer.sample_buffer()
        ...
        new_raw_data = torch.tensor(new_raw_data).to(
            self.q_network_eval.device)
        new_nav_data = torch.tensor(new_nav_data).to(
            self.q_network_eval.device)
        ...
        if self.double:
            q_pred = self.q_network_eval.forward(
                visual_state, raw_state, nav_data)[indexes, action]
            q_next = self.q_network_target.forward(
                new_visual_state, new_raw_data, new_nav_data)
            ...
            q_pred = self.q_network_eval.forward(
                visual_state, raw_state, nav_data)[indexes, action]
            q_next = self.q_network_target.forward(
                new_visual_state, new_raw_data, new_nav_data).max(dim=1)[0]
